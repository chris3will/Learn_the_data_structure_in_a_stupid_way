...
//	辅助函数模板:
	...
	ChildSiblingTreeNode<ElemType> *CopyTreeHelp(ChildSiblingTreeNode<ElemType> *r);	
		// 复制森林
...
{
	if (cur == NULL) return FAIL;								// cur不存在
	if (i <= 0 || i > NodeDegree(cur) + 1) return FAIL;			// i位置错
...
template <class ElemType>
ChildSiblingTreeNode<ElemType> *ChildSiblingForest<ElemType>::CopyTreeHelp(ChildSiblingTreeNode<ElemType> *r)
